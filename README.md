...
- **Four banks so far**: **Rain** (six surfaces -- umbrella, puddle, concrete, terrace, plastic tarpaulin, metal wheelbarrow -- a steady wash through to increasingly percussive and metallic), **Birds** (forest ambience, dawn chorus, evening birds), **Insects** (nocturnal insects, crickets/frogs meadow, a close field cricket, a lone night grasshopper), and **Ocean** (two wave textures, one underwater hydrophone recording of a waterfall). All CC0, all placeholders pending original field recordings (see [docs/SOURCES.md](docs/SOURCES.md)). Adding a further bank is data, not a rewrite -- see `src/Field.h`. A cicada clip in Insects and a whole Body bank (heartbeat, breathing) were both tried and dropped: the cicada read as off-putting, and Body just wasn't interesting. Ocean's third clip took several tries -- a humpback whale song read as too weak, a bottlenose dolphin's clicks were unpleasant on a loop, and a Weddell seal trill was tuned but never shipped once its license turned out unverified -- before landing on the underwater recording.
