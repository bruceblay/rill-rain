...
- **Two banks so far**: **Rain** (six surfaces -- umbrella, puddle, concrete, terrace, plastic tarpaulin, metal wheelbarrow -- a steady wash through to increasingly percussive and metallic) and **Birds** (forest ambience, dawn chorus, evening birds). All CC0-licensed placeholders (see [docs/SOURCES.md](docs/SOURCES.md)), expected to be replaced with original field recordings. Adding a further bank (body sounds, insects) is data, not a rewrite -- see `src/Field.h`.
...
- **Five punch-in effects** -- Pitch Wobble, Delay Throw, Crush, Reverb, Smear -- one at a time, semi-random, self-clearing after a bar or two, each evolving across its own window rather than sitting at one flat setting. Same direction as Rill Drums' punch-in effects. All five apply to whichever bank is active.
